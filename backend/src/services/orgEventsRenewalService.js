// ═══════════════════════════════════════════════════════════════
// ORG EVENTS RENEWAL SERVICE
//
// The Org Events plan is a $199 one-time payment that lasts 1 year.
// From Year 2 onward, users pay $65/year to keep their account active.
//
...
//
// Required env vars:
//   STRIPE_ORG_EVENTS_RENEWAL_PRICE — price_xxx for the $65/year renewal product
//   FRONTEND_URL
//   RESEND_API_KEY
// ═══════════════════════════════════════════════════════════════
...
const sendRenewalEmail = async ({ to, name, daysLeft, renewalUrl, expiresAt }) => {
  ...
  const subject = isExpired
    ? '⚠️ Your Outbound Impact Org Events plan has expired'
    : isUrgent
...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.10);">
  <div style="background:linear-gradient(135deg,${isExpired ? '#CC3333,#FF6B6B' : isUrgent ? '#FFB020,#FF8C42' : '#00C49A,#7B4FD6'});padding:32px 40px;text-align:center;">
    <h1 style="color:#fff;margin:0;font-size:22px;">
      ${isExpired ? '⚠️ Plan Expired' : isUrgent ? '⏰ Renewal Reminder' : '📅 Plan Renewal Coming Up'}
    </h1>
    <p style="color:rgba(255,255,255,.85);margin:8px 0 0;font-size:14px;">Outbound Impact — Org Events Plan</p>
  </div>
  <div style="padding:36px 40px;">
    ...
    <p style="color:#374151;font-size:15px;line-height:1.7;margin:0 0 24px;">
      Your Org Events plan expired on <strong>${expiryStr}</strong>. Renew now to restore full access to your QR codes, analytics, and content.
    </p>
    ` : `
    <p style="color:#374151;font-size:15px;line-height:1.7;margin:0 0 24px;">
      Your Org Events plan ${isUrgent ? `expires in <strong>${daysLeft} day${daysLeft === 1 ? '' : 's'}</strong>` : `is due for renewal on <strong>${expiryStr}</strong>`}.
      Renew for just <strong>$65/year</strong> to keep your account active.
    </p>
    `}
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#F8FAFF;border:1px solid #E0E7FF;border-radius:10px;overflow:hidden;margin-bottom:28px;">
      <tr><td style="padding:14px 20px;border-bottom:1px solid #E0E7FF;font-size:14px;color:#6B7280;">Plan</td>
          <td style="padding:14px 20px;border-bottom:1px solid #E0E7FF;font-size:14px;font-weight:700;color:#0B1220;">Org Events</td></tr>
      <tr><td style="padding:14px 20px;border-bottom:1px solid #E0E7FF;font-size:14px;color:#6B7280;">Renewal Price</td>
          <td style="padding:14px 20px;border-bottom:1px solid #E0E7FF;font-size:14px;font-weight:700;color:#00C49A;">USD $65 / year</td></tr>
    ...
    <div style="text-align:center;margin:32px 0;">
      <a href="${renewalUrl}"
         style="display:inline-block;padding:16px 40px;background:linear-gradient(135deg,#00C49A,#7B4FD6);color:#fff;text-decoration:none;border-radius:12px;font-weight:700;font-size:17px;box-shadow:0 4px 20px rgba(0,196,154,.30);">
        Renew Now — USD $65/year →
      </a>
    </div>
...
const createRenewalPaymentLink = async (user) => {
  ...
  if (!renewalPriceId) {
    throw new Error('STRIPE_ORG_EVENTS_RENEWAL_PRICE not configured');
  }
...
const runRenewalCheck = async () => {
  console.log('🔄 [Org Events Renewal] Starting daily renewal check...');

  const renewalPriceId = process.env.STRIPE_ORG_EVENTS_RENEWAL_PRICE;
  if (!renewalPriceId) {
    console.warn('⚠️ [Org Events Renewal] STRIPE_ORG_EVENTS_RENEWAL_PRICE not set — skipping');
    return;
  }

  const now = new Date();

  // Fetch all active ORG_EVENTS users with an expiry date
  const orgEventsUsers = await prisma.user.findMany({
    where: {
      role:                'ORG_EVENTS',
      orgEventsExpiresAt:  { not: null },
      subscriptionStatus:  { not: 'suspended' },
    },
    select: {
      id: true, email: true, name: true,
      orgEventsExpiresAt: true, subscriptionStatus: true,
    },
  });

  console.log(`   Found ${orgEventsUsers.length} ORG_EVENTS users to check`);

  for (const user of orgEventsUsers) {
    const expiresAt = new Date(user.orgEventsExpiresAt);
      ...
      // ── 7 days after expiry — suspend access ─────────────────
        ...
        try {
          await resend.emails.send({
            ...
            subject: `🔒 ORG_EVENTS account suspended: ${user.email}`,
            html:    `<p>${user.name} (${user.email}) has not renewed their Org Events plan. Their account has been suspended. They can reactivate by renewing at ${FRONTEND}/dashboard/settings.</p>`,
          });
  ...
  console.log('✅ [Org Events Renewal] Daily check complete');
};

// ─────────────────────────────────────────────────────────────
// START CRON — daily at 09:00 UTC
// ─────────────────────────────────────────────────────────────
const startOrgEventsRenewalCron = () => {
  cron.schedule('0 9 * * *', async () => {
    await runRenewalCheck().catch(err =>
      console.error('❌ [Org Events Renewal] Cron error:', err.message)
    );
  });

  console.log('⏰ Org Events renewal cron scheduled (daily at 09:00 UTC)');
};

module.exports = { startOrgEventsRenewalCron, runRenewalCheck };